...
void GSDDumpWriter::writeProperties(const GSDDumpWriter::GSDFrame& frame)
    ...
    if (frame.particle_data.confidence.size() != 0)
        {
        assert(frame.particle_data.confidence.size() == N);

        m_exec_conf->msg->notice(10) << "GSD: writing particles/confidence" << endl;
        retval = gsd_write_chunk(&m_handle,
                                 "particles/confidence",
                                 GSD_TYPE_FLOAT,
                                 N,
...
void GSDDumpWriter::populateLocalFrame(GSDDumpWriter::GSDFrame& frame, uint64_t timestep)
    ...
    if (all_default[gsd_flag::particles_confidence]
        && !(m_nframes > 0 && m_nondefault["particles/confidence"]))
        {
