...
while IFS=, read -r Np runtime noise_Q kHT2 kklino iftaxis ifkk ifok iflarge depth
do
    echo "Running ${python_script} with parameters: $Np $runtime $noise_Q $kHT2 $kklino $iftaxis $ifkk $ifok $iflarge $depth"

    source /home/wanxuan/venvpheromone/bin/activate
    # Run the Python script with parameters
    logname="log${Np}-${runtime}-${noise_Q}-${kHT2}-${kklino}-${iftaxis}-${ifkk}-${ifok}-${iflarge}-${depth}"
    echo "logname is ${logname}"
    nohup python -u "$python_script" "$Np" "$runtime" "$noise_Q" "$kHT2" "$kklino" "$iftaxis" "$ifkk" "$ifok" "$iflarge" "$depth" > ${logname} &
