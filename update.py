...
class MixedActiveRotationalDiffusionRunTumble(Updater):
    r"""Updater to introduce rotational diffusion and run and tumble with a mixed active force.

    Args:
        trigger (hoomd.trigger.trigger_like): Select the timesteps to update
            rotational diffusion and tumble.
        mixed_active_force (hoomd.md.force.MixedActive): The active force associated with
            the updater can be any subclass of the class
            `hoomd.md.force.MixedActive`.
        rotational_diffusion (hoomd.variant.variant_like): The rotational
            diffusion as a function of time.
    ...
    def _attach_hook(self):
        ...
        # attached then the active force is not a part of the simulation, and we
        # should error.
        if not self.mixed_active_force._attached:
            raise SimulationDefinitionError(
                "MixedActive force for MixedActiveRotationalDiffusionRunTumble object does not "
                "belong to the simulation integrator.")
        if self.mixed_active_force._simulation is not self._simulation:
            raise SimulationDefinitionError(
                "MixedActive force for MixedActiveRotationalDiffusionRunTumble object belongs to "
                "another simulation.")
    ...
    def _handle_removed_dependency(self, mixed_active_force):
        ...
        if sim is not None:
            sim._operations.updaters.remove(self)
        super()._handle_removed_dependency(mixed_active_force)

    def _setattr_param(self, attr, value):
        if attr == "mixed_active_force":
            raise ValueError("mixed_active_force is not settable after construction.")
        super()._setattr_param(attr, value)
